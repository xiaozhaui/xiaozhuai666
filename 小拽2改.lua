...
-- === Floating Window UILib Wrapper (replaces external lib) ===
local function __MakeFloatingUILib()
    ...
    local function createRow(labelText, onToggle)
        ...
        btn.Size = UDim2.fromOffset(64, 24); btn.Position = UDim2.new(1, -74, 0.5, -12)
        btn.BackgroundColor3 = Color3.fromRGB(40, 120, 255)
        btn.TextColor3 = Color3.fromRGB(255,255,255); btn.TextSize = 14
        btn.Font = Enum.Font.GothamBold; btn.Text = "开"
        btn.ZIndex = 123; btn.Parent = row
        local bcorner = Instance.new("UICorner"); bcorner.CornerRadius = UDim.new(0, 6); bcorner.Parent = btn

        local rowHit = Instance.new("TextButton"); rowHit.BackgroundTransparency = 1; rowHit.AutoButtonColor = false
        rowHit.Size = UDim2.fromScale(1, 1); rowHit.Text = ""; rowHit.ZIndex = 122; rowHit.Parent = row
    ...
    function UILike:NewWindow(titleText)
        ...
        function windowObj:NewSection(name)
            ...
            function sectionObj:CreateToggle(label, callback)
                createRow(label, callback)
            end
            -- Keep API compatibility if code tries to call :CreateButton later (no-op default)
            function sectionObj:CreateButton(label, callback)
                ...
                labBtn.Font = Enum.Font.Gotham; labBtn.TextSize = 14
                labBtn.TextXAlignment = Enum.TextXAlignment.Left
                labBtn.TextColor3 = Color3.fromRGB(30,30,30); labBtn.Text = label or "按钮"
                labBtn.ZIndex = 123; labBtn.Parent = row

                local btn = Instance.new("TextButton")
                btn.Size = UDim2.fromOffset(80, 24); btn.Position = UDim2.new(1, -90, 0.5, -12)
                btn.BackgroundColor3 = Color3.fromRGB(20,160,140)
                btn.TextColor3 = Color3.fromRGB(255,255,255); btn.TextSize = 14
                btn.Font = Enum.Font.GothamBold; btn.Text = "执行"
                btn.ZIndex = 123; btn.Parent = row
                local bcorner = Instance.new("UICorner"); bcorner.CornerRadius = UDim.new(0,6); bcorner.Parent = btn
    ...
    coroutine.wrap(function()
        while boundProtect do
            ...
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- 与原逻辑一致（保持不变；若原版用了 vector.clamp，这里仍保留你的实现语义）
